    ...
    while (subnode != node->tail) {
        if (subnode->type != NULL) {
            if (subnode->desired_type != NULL) {
                type_checker_type_convertable_result_t convertable_result = type_checker_type_convertable(
                    subnode->desired_type,
                    subnode->type
                );
...
//                    );
                    ...
                    char * desired_str = type_checker_type_stringify(subnode->desired_type);
                    char * type_str = type_checker_type_stringify(subnode->type);

                    free_list_push(&free_list, desired_str, pkcc_free);
                    free_list_push(&free_list, type_str, pkcc_free);
