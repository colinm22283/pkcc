...
__NORETURN static inline void fatal_line_full_error(line_buffer_t * line_buffer, file_name_entry_t * file_name, const char * message, size_t line, size_t position) {
    fatal_line_range_error(line_buffer, file_name, message, line, position, strlen(line_buffer->lines[line].line));
}